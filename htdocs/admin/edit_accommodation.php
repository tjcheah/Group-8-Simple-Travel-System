<?php
	...
	if($_GET){
		$sql = "SELECT * FROM hotel WHERE hotel_id = ".$_GET['id'];
		$hotel = mysqli_query($conn, $sql);
		$row = mysqli_fetch_array($hotel);
		...
		<div class="row my-4">
		  <div class="col-10">
			<div class="tab-content" id="v-pills-tabContent">
			  <div class="tab-pane fade show active" id="v-pills-accommodation" role="tabpanel" aria-labelledby="v-pills-accommodation-tab">
				<h2><?php echo $title; ?></h2>
				<form method="POST" action="../php/update_accommodation.php">
					<?php if(isset($row['hotel_id'])){?>
						<input type="hidden" name="hotel_id" value="<?php echo $row['hotel_id'] ?>">
					<?php } ?>
				  <div class="form-group">
					<label for="hotelName">Accommodation Name: </label>
					<input type="text" class="form-control" id="hotelName" name="hotelName" value="<?php echo $row['hotelName']; ?>" required>
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-group">
					...
					<textarea type="text" class="form-control" rows="5" id="hotelDescript" name="hotelDescript" required><?php echo $row['hotelDescript']; ?></textarea>
				  </div>
